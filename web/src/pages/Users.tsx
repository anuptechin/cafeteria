...
export function Users() {
  ...
  return (
    <div className="space-y-6">
      ...
      <CreateUser actorRole={me.role} onCreated={reload} />
      ...
      {loading && !data ? (
        ...
        <Card title={`Accounts (${data?.length ?? 0})`} className="!p-0">
          <div className="divide-y">
            {data?.map((u) => (
              <UserRow key={u.id} u={u} actorRole={me.role} me={me.id === u.id} onChange={reload} />
            ))}
          </div>
        </Card>
      )}
...
function CreateUser({ actorRole, onCreated }: { actorRole: Role; onCreated: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setBusy(true);
    setErr(null);
    const res = await api.createUser({ name: name.trim(), username: username.trim(), password, role });
    setBusy(false);
